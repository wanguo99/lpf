...
int32 PMC_TM_Cache_Init(pmc_tm_cache_t **cache)
{
    ...
    for (uint32 i = 0; i < PMC_TM_MAX_COUNT; i++) {
        ...
        /* 创建读写锁 */
        ret = OSAL_MutexCreate(&entry->rwlock, "tm_lock", 0);
        if (ret != OS_SUCCESS) {
...
/* 计算新鲜度 */
static pmc_tm_freshness_t calculate_freshness(uint64 timestamp_us, uint32 validity_ms)
{
    uint64 now_us = OSAL_GetTimeUs();
    uint64 age_ms = (now_us - timestamp_us) / 1000;
                          ...
                          const uint8 *data, uint32 size, uint32 validity_ms)
{
    ...
    /* 写锁 */
    int32 ret = OSAL_MutexLock(entry->rwlock, OSAL_WAIT_FOREVER);
    if (ret != OS_SUCCESS) {
        return ret;
    }
    ...
    entry->timestamp_us = OSAL_GetTimeUs();
...
int32 PMC_Status_Init(pmc_system_status_t **status)
{
    ...
    /* 创建互斥锁 */
    ret = OSAL_MutexCreate(&(*status)->mutex, "status_lock", 0);
    if (ret != OS_SUCCESS) {
...
int32 PMC_Status_Write(pmc_system_status_t *status, const pmc_system_status_t *new_status)
{
    ...
    int32 ret = OSAL_MutexLock(status->mutex, OSAL_WAIT_FOREVER);
    if (ret != OS_SUCCESS) {
        return ret;
    }
...
int32 PMC_Heartbeat_Init(pmc_process_heartbeat_t **heartbeat)
{
    ...
    /* 初始化心跳 */
    for (uint32 i = 0; i < PMC_PROCESS_MAX; i++) {
...
int32 PMC_Heartbeat_Update(pmc_process_heartbeat_t *heartbeat, pmc_process_id_t process_id)
{
    ...
    heartbeat->heartbeat_us[process_id] = OSAL_GetTimeUs();
    return OS_SUCCESS;
}
                           ...
                           uint32 timeout_ms, bool *alive)
{
    ...
    uint64 now_us = OSAL_GetTimeUs();
    uint64 last_hb = heartbeat->heartbeat_us[process_id];
    uint64 age_ms = (now_us - last_hb) / 1000;
...
int32 PMC_Log_Write(pmc_log_ringbuffer_t *log_ring, const char *log_entry)
{
    ...
    uint32 write_idx = log_ring->write_index;
    uint32 next_idx = (write_idx + 1) % PMC_LOG_ENTRY_COUNT;

    /* 检查是否满 */
    if (next_idx == log_ring->read_index) {
        return OS_ERROR;  /* 缓冲区满 */
    }
...
int32 PMC_Log_Read(pmc_log_ringbuffer_t *log_ring, char *log_entry, uint32 size)
{
    ...
    uint32 read_idx = log_ring->read_index;

    /* 检查是否空 */
    if (read_idx == log_ring->write_index) {
        return OS_ERROR;  /* 缓冲区空 */
    }
