/**
 * @file aconfig_pmc_h200_100p_am625.c
 * @brief PMC H200-100P-AM625 Platform AConfig Configuration
 * @note Product layer: PMC-specific configuration tables
...
static const aconfig_tc_entry_t g_pmc_tc_entries[] = {
    /* 电源控制 → BMC */
    {
...
static const aconfig_tm_entry_t g_pmc_tm_entries[] = {
    /* 温度遥测 → 传感器 */
    {
...
static const aconfig_config_table_t g_pmc_aconfig_table = {
    .name = "PMC_H200",
    .hwid_count = 0,      // 支持所有 HWID
    .hwid_list = NULL,

    /* 稀疏数组：只存储实际定义的功能 */
    .tc_entries = g_pmc_tc_entries,
    .tc_count = sizeof(g_pmc_tc_entries) / sizeof(aconfig_tc_entry_t),

    .tm_entries = g_pmc_tm_entries,
    .tm_count = sizeof(g_pmc_tm_entries) / sizeof(aconfig_tm_entry_t)
};

/**
 * @brief 初始化 PMC AConfig 配置
 * @return OSAL_SUCCESS 成功，其他值失败
 */
int32_t PMC_ACONFIG_Init(void)
{
    int32_t ret;
    ...
    ret = ACONFIG_init();
    if (OSAL_SUCCESS != ret) {
        LOG_ERROR("PMC_ACONFIG", "ACONFIG_init failed, ret=%d", ret);
        return ret;
    }

    /* 注册 PMC 配置表（新格式） */
    ret = ACONFIG_register_table(&g_pmc_aconfig_table);
    if (OSAL_SUCCESS != ret) {
        LOG_ERROR("PMC_ACONFIG", "ACONFIG_register_table failed, ret=%d", ret);
        return ret;
    }

    LOG_INFO("PMC_ACONFIG", "Initialized (optimized format)");
    LOG_INFO("PMC_ACONFIG", "  TC entries: %u (sparse array)",
             g_pmc_aconfig_table.tc_count);
    LOG_INFO("PMC_ACONFIG", "  TM entries: %u (sparse array)",
             g_pmc_aconfig_table.tm_count);
 ...
 *
 * 5. 注册 API 变化：
 *    旧：ACONFIG_RegisterTableLegacy(&g_pmc_aconfig_table)
 *    新：ACONFIG_register_table(&g_pmc_aconfig_table)
 *
 * 内存占用对比：
