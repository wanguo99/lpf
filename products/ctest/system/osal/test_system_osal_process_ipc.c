...
static void test_shared_memory_between_processes(void) {
	...
	OSAL_snprintf(shm_name, sizeof(shm_name), "/test_proc_shm_%d", OSAL_getpid());
...
static void test_semaphore_process_sync(void) {
	...
	OSAL_snprintf(shm_name, sizeof(shm_name), "/test_sem_sync_%d", OSAL_getpid());
	shm_fd = OSAL_shm_open(shm_name, OSAL_O_CREAT | OSAL_O_RDWR, 0666);
	if (shm_fd < 0) {
		OSAL_printf("[ SKIP     ] Shared memory not available\n");
		return;
...
static void register_system_osal_process_ipc_tests(void)
{
