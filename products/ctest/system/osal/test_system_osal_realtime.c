...
static void test_rt_thread_priority_enforcement(void) {
	...
	/* Create threads */
	ret = OSAL_pthread_create(&low_prio_thread, &low_attr, priority_thread_func, &low_ctx);
	if (ret != 0) {
		OSAL_printf("[ SKIP     ] Cannot create low priority thread (need root for RT)\n");
		OSAL_sem_destroy(&start_sem);
		return;
	}

	ret = OSAL_pthread_create(&high_prio_thread, &high_attr, priority_thread_func, &high_ctx);
	if (ret != 0) {
...
static void test_scheduler_policy_interactions(void) {
	...
	for (i = 0; i < 3; i++) {
		ret = OSAL_pthread_create(&threads[i], NULL, sched_thread_func, &contexts[i]);
		if (ret != 0) {
...
static void register_system_osal_realtime_tests(void)
{
