...
static void *file_writer_func(void *arg)
{
    ...
    OSAL_snprintf(filename,
                  sizeof(filename),
                  "/tmp/test_write_%lu.txt",
...
static void test_fd_cleanup(void)
{
    ...
    for (i = 0; i < 100; i++) {
        fd = OSAL_open(test_file,
                       OSAL_O_CREAT | OSAL_O_WRONLY | OSAL_O_TRUNC,
                       0644);
