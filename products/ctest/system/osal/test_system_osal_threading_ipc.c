...
static void test_shared_memory_multithread(void) {
	...
	OSAL_snprintf(ctx.shm_name, sizeof(ctx.shm_name), "/test_shm_mt_%d", OSAL_getpid());
	ctx.shm_size = 4096;
	...
	ctx.shm_counter = (osal_atomic_uint32_t*)((char*)ctx.shm_ptr + sizeof(osal_mutex_t));
...
static void test_condition_wakeup_patterns(void) {
	...
	for (i = 0; i < 5; i++) {
		ret = OSAL_pthread_create(&waiter_threads[i], NULL, waiter_func, &waiter_ctx);
		TEST_ASSERT_EQUAL(0, ret);
	}
...
static void register_system_osal_threading_ipc_tests(void)
{
