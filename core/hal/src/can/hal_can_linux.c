...
int32_t hal_can_init(const hal_can_config_t *config, hal_can_handle_t *handle)
{
    ...
    osal_snprintf(lock_file, OSAL_sizeof(lock_file), HAL_CAN_LOCK_PATH_FMT,
                  config->interface);
    ret = _hal_linux_lock_init(&impl->lock, lock_file, "HAL_CAN");
    if (ret != OSAL_SUCCESS)
        goto err_free;

    ret = _hal_linux_lock_acquire(&impl->lock, HAL_CAN_LOCK_TIMEOUT_MS,
                                  "HAL_CAN");
    if (ret != OSAL_SUCCESS)
        goto err_lock;
    ...
    *handle = (hal_can_handle_t)impl;
    _hal_linux_lock_release(&impl->lock);

    LOG_INFO("HAL_CAN",
             "Initialized successfully: %s @ %u bps (with process/thread "
             "protection)",
             config->interface, config->baudrate);
    return OSAL_SUCCESS;

err_close:
    _hal_linux_close_fd(&impl->sockfd);
err_unlock:
    _hal_linux_lock_release(&impl->lock);
err_lock:
    _hal_linux_lock_deinit(&impl->lock);
err_free:
    osal_free(impl);
    return ret;
}
