...
int32_t hal_i2c_open(const hal_i2c_config_t *config, hal_i2c_handle_t *handle)
{
    ...
    ret = _hal_linux_lock_init(&impl->lock, lock_file, "HAL_I2C");
    if (ret != OSAL_SUCCESS)
        goto err_free;

    ret = _hal_linux_lock_acquire(&impl->lock, HAL_I2C_LOCK_TIMEOUT_MS,
                                  "HAL_I2C");
    if (ret != OSAL_SUCCESS)
        goto err_lock;
    ...
    *handle = (hal_i2c_handle_t)impl;
    ...
    LOG_INFO("HAL_I2C",
             "Opened device %s (fd=%d, with process/thread protection)",
