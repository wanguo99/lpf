...
typedef struct
{
    ...
    char pmc_ip[16];             /* PMC IP 地址 */
    uint16_t pmc_port;           /* PMC 端口号 */
...
int32_t pmc_eth_init(const pdl_pmc_config_t *config, void **handle)
{
    ...
    /* 分配上下文 */
    ctx = (pmc_eth_context_t *)OSAL_malloc(OSAL_sizeof(pmc_eth_context_t));
    if (!ctx)
    {
        return OSAL_ERR_NO_MEMORY;
    }

    OSAL_memset(ctx, 0, OSAL_sizeof(pmc_eth_context_t));
    OSAL_strncpy(ctx->pmc_ip, config->pmc_ip, OSAL_sizeof(ctx->pmc_ip) - 1);
    ctx->pmc_port = config->pmc_port;
    ...
    if (ctx->sockfd < 0)
    {
        LOG_ERROR("PDL_PMC", "Failed to create socket");
    ...
    server_addr.sin_port = OSAL_htons(config->pmc_port);

    /* 转换 IP 地址 */
    ret = OSAL_inet_pton(OSAL_AF_INET, config->pmc_ip, &server_addr.sin_addr);
    if (ret != 1)
    {
        LOG_ERROR("PDL_PMC", "Invalid IP address: %s", config->pmc_ip);
    ...
    {
        ctx->connected = true;
        LOG_INFO("PDL_PMC", "Connected to %s:%u", config->pmc_ip, config->pmc_port);
    }
    else
    {
        LOG_WARN("PDL_PMC", "Failed to connect to %s:%u, will retry later",
                 config->pmc_ip, config->pmc_port);
        ctx->connected = false;
    }
...
{
    pmc_eth_context_t *ctx = (pmc_eth_context_t *)handle;
...
{
    pmc_eth_context_t *ctx = (pmc_eth_context_t *)handle;
    uint8_t send_buf[PMC_MAX_MSG_SIZE + 16];  /* 预留协议头空间 */
    size_t send_len;
    int32_t ret;
    ...
    {
        LOG_ERROR("PDL_PMC", "Not connected");
        return OSAL_ERR_GENERIC;
    }
    ...
    if (ret < 0)
    {
        LOG_ERROR("PDL_PMC", "Failed to send message");
        ctx->connected = false;
        return OSAL_ERR_GENERIC;
    }
    ...
    {
        LOG_ERROR("PDL_PMC", "Partial send (%d/%zu bytes)", ret, send_len);
        ctx->connected = false;
        return OSAL_ERR_GENERIC;
    }
...
{
    pmc_eth_context_t *ctx = (pmc_eth_context_t *)handle;
    uint8_t recv_buf[PMC_MAX_MSG_SIZE + 16];
    int32_t ret;
    ...
    if (ret < 0)
    {
        LOG_ERROR("PDL_PMC", "Failed to receive header");
        ctx->connected = false;
        return OSAL_ERR_GENERIC;
    }
    ...
    if (ret == 0)
    {
        LOG_INFO("PDL_PMC", "Connection closed by peer");
        ctx->connected = false;
        return OSAL_ERR_GENERIC;
    }
    ...
    if (ret < 16)
    {
        LOG_ERROR("PDL_PMC", "Received header too short (%d bytes)", ret);
        return OSAL_ERR_INVALID_PARAM;
    }
    ...
    {
        LOG_ERROR("PDL_PMC", "Invalid magic number");
        return OSAL_ERR_INVALID_PARAM;
    }
    ...
    {
        LOG_ERROR("PDL_PMC", "Payload too large (%u bytes)", msg->payload_len);
        return OSAL_ERR_INVALID_PARAM;
    }
    ...
    if (msg->payload_len > 0)
    {
        ...
        if (ret < 0)
        {
            LOG_ERROR("PDL_PMC", "Failed to receive payload");
            ctx->connected = false;
            return OSAL_ERR_GENERIC;
        }
        ...
        {
            LOG_ERROR("PDL_PMC", "Incomplete payload (%d/%u bytes)", ret, msg->payload_len);
            return OSAL_ERR_INVALID_PARAM;
        }
    }
    ...
    {
        LOG_ERROR("PDL_PMC", "CRC mismatch (recv=0x%08X, calc=0x%08X)", crc_recv, crc_calc);
        return OSAL_ERR_INVALID_PARAM;
    }
...
/*
 * Ethernet 接口的 ops 结构定义（导出供 pdl_pmc.c 使用）
 */
const pdl_pmc_ops_t pmc_eth_ops = {
	.init = (int32_t (*)(const void *, void **))pmc_eth_init,
	.deinit = pmc_eth_deinit,
	.send = pmc_eth_send,
	.recv = pmc_eth_recv,
	.is_connected = pmc_eth_is_connected,
};
