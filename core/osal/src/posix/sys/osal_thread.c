                             ...
                             void *arg)
{
    ...
    if (attr != NULL)
    {
        ...
        /* 设置分离状态 */
        int detach_state = attr->detached ? PTHREAD_CREATE_DETACHED : PTHREAD_CREATE_JOINABLE;
        pthread_attr_setdetachstate(&pthread_attr, detach_state);

        /* 设置调度属性继承 */
        int inherit = attr->inherit_sched ? PTHREAD_INHERIT_SCHED : PTHREAD_EXPLICIT_SCHED;
    ...
    /* 创建线程 */
    int32_t ret = OSAL_pthread_create(thread, &pthread_attr, func, arg);
