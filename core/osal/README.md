...
# 单独编译OSAL
cd build/release && make osal -j$(nproc)
```
...
# 运行 OSAL 测试
./build/debug/bin/unit-test -L OSAL           # 所有 OSAL 测试
./build/debug/bin/unit-test -m test_osal_task # 任务测试
./build/debug/bin/unit-test -i                # 交互式菜单
```
