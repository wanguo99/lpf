...
2. `aa8c12f` - 文件锁实现
3. `75fda9b` - 更新full_test_defconfig
...
13. `2f5be5a` - 更新full_test_defconfig启用所有已修复的测试
...
| 配置文件 | 用途 | 启用模块 |
|---------|------|---------|
| `osal_test_defconfig` | OSAL单元测试 | OSAL |
| `hal_test_defconfig` | HAL单元测试 | OSAL + HAL |
| `pdl_test_defconfig` | PDL单元测试 | OSAL + HAL + PRL + PDL |
| `prl_test_defconfig` | PRL单元测试 | OSAL + HAL + PRL |
| `aconfig_test_defconfig` | ACONFIG单元测试 | OSAL + PCONFIG + ACL |
| `pconfig_test_defconfig` | PCONFIG单元测试 | OSAL + HAL + PRL + PDL + PCL |
| `full_test_defconfig` | 完整测试套件 | 所有测试模块 |
...
# 编译并运行完整测试
python3 build.py config full_test && python3 build.py build
./_build/bin/ems-test -a
