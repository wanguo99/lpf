...
int32_t perf_calculate_stats(perf_context_t *ctx, perf_stats_t *stats) {
    ...
    if (sorted) {
        ...
        OSAL_qsort(sorted, ctx->sample_count, sizeof(double), compare_double);
...
perf_context_t* perf_context_create(const char *name,
    ...
    perf_context_t *ctx = (perf_context_t*)malloc(sizeof(perf_context_t));
    if (!ctx) {
        return NULL;
    }

    ctx->samples = (double*)malloc(sizeof(double) * max_samples);
    if (!ctx->samples) {
        free(ctx);
        return NULL;
    }
...
int32_t perf_calculate_stats(perf_context_t *ctx, perf_stats_t *stats) {
    ...
    /* 计算百分位数（需要排序） */
    double *sorted = (double*)malloc(sizeof(double) * ctx->sample_count);
    if (sorted) {
        memcpy(sorted, ctx->samples, sizeof(double) * ctx->sample_count);
        qsort(sorted, ctx->sample_count, sizeof(double), compare_double);
...
int32_t perf_compare_baseline(perf_context_t *ctx,
    ...
    memset(result, 0, sizeof(perf_result_t));
    ...
    /* 判断是否通过基准测试 */
    double abs_diff_percent = fabs(result->baseline_diff_percent);
    result->baseline_passed = (abs_diff_percent <= baseline->tolerance_percent);
...
void perf_print_result(const perf_result_t *result) {
    ...
    const char *status = result->baseline_passed ? "PASS" : "FAIL";
    const char *symbol = result->baseline_passed ? "✓" : "✗";
