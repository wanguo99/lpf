...
`kernel/lpf-core/peripheral/` and remain integrated through the Core module
...
`kernel/lpf-core/include/` and must not be included outside the runtime
...
MCU and LED service implementations live under `kernel/lpf-core/peripheral/`.
...
`kernel/lpf-core/peripheral/mcu/` and are selected through
