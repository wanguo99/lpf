...
void lpf_protocol_init_header(lpf_protocol_header_t *hdr, uint8_t dev_type,
			      ...
			      uint8_t flags)
{
	osal_memset(hdr, 0, OSAL_sizeof(lpf_protocol_header_t));

	/* 多字节字段使用网络字节序（大端），确保跨平台兼容性 */
	hdr->magic = osal_htons(LPF_PROTOCOL_MAGIC);
	hdr->version = LPF_PROTOCOL_VERSION;
	hdr->dev_type = dev_type;
	hdr->msg_type = msg_type;
	hdr->flags = flags;
	hdr->length = osal_htons(payload_len);
	hdr->seq = osal_htonl(lpf_protocol_get_next_seq());
	hdr->timestamp = osal_htonl(lpf_protocol_get_timestamp());
	/* crc16 将在 lpf_protocol_set_packet_crc() 中转换 */
}
				 ...
				 uint8_t expected_dev_type)
{
	...
	if (magic != LPF_PROTOCOL_MAGIC) {
		return OSAL_EPROTO; /* 协议错误：魔数不匹配 */
	}

	if ((hdr->version >> 4) != LPF_PROTOCOL_VERSION_MAJOR) {
		return OSAL_EPROTO; /* 协议错误：版本不匹配 */
	}

	if (expected_dev_type != LPF_PROTOCOL_DEV_TYPE_INVALID &&
	    hdr->dev_type != expected_dev_type) {
		return OSAL_EINVAL; /* 无效的设备类型 */
	}

	if (length > LPF_PROTOCOL_MAX_PAYLOAD_SIZE) {
...
{
	lpf_protocol_header_t *hdr = (lpf_protocol_header_t *)packet;
...
{
	const lpf_protocol_header_t *hdr = (const lpf_protocol_header_t *)packet;
