...
# 1. 加载预定义配置
python3 build.py config <config_name>    # 例如：ccm_development, full_test
...
#### 测试配置
- `full_test` - 全栈测试（所有模块、所有功能）
- `osal_test_x86` / `osal_test_arm64` - OSAL 单元测试
- `hal_test_linux` - HAL 单元测试
- `pdl_test` - PDL 单元测试
- `prl_test` - PRL 协议层测试
- `aconfig_test` - ACONFIG 单元测试
- `acl_test` - ACL 单元测试
- `pcl_test` - PCL 单元测试
- `system_test` - 系统集成测试
- `stress_test` - 压力测试
