...
struct pdm_device {
    ...
    u32 capabilities;               // 能力位掩码
    int requested_id;               // DTS reg/pdm,id 属性
    int id;                         // 从 IDA 分配的 ID
    const char *compatible;         // 主 compatible 字符串
    enum pdm_manager_device_state state;  // UNBOUND, BOUND, ERROR
    void *config_data;              // 驱动特定配置
    /* ... */
};
```
...
struct pdm_device *pdm_device_alloc(size_t config_size)
{
    ...
    // 初始化状态
    pdm_dev->state = PDM_MANAGER_DEVICE_STATE_UNBOUND;
...
步骤 1: pdm_device_ids_init()
  ...
  位置：pdm_device.c:98-108
  
  代码：
  ida_init(&led_allocator.ida);
  ida_init(&mcu_allocator.ida);

步骤 2: bus_register(&pdm_bus_type)
  功能：在 Linux 驱动核心中注册 PDM 总线
  位置：pdm_bus.c:240-250
...
步骤 3: pdm_of_bus_init()
  功能：注册平台驱动以枚举 DTS 中的 PDM 总线节点
  位置：pdm_of_bus.c:131-138
  
  代码：
  platform_driver_register(&pdm_of_bus_driver);
  
  匹配：compatible = "pdm,bus"
...
步骤 5: pdm_driver_entries_init()
  功能：初始化所有驱动
  位置：pdm_driver_registry.c:33-48
...
pdm_module_init()
    ...
    │
    ├─→ bus_register()               [PDM 总线注册]
    │       │
    │       └─→ /sys/bus/pdm/ 创建
    │
    ├─→ pdm_of_bus_init()           [平台驱动注册]
    │       │
    │       └─→ 等待 DTS 匹配
    │
...
1. 平台设备匹配
   ...
   → 匹配 pdm_of_bus_driver (compatible = "pdm,bus")
   → 调用 pdm_of_bus_probe()
...
5. 匹配算法触发
   对于每个 state=UNBOUND 的 pdm_device：
...
│
└── compat/                        # 兼容性层
    ├── pdm_compat_i2c.c           # I2C 兼容性
    ├── pdm_compat_spi.c           # SPI 兼容性
    └── pdm_compat_serdev.c        # Serdev 兼容性
```
